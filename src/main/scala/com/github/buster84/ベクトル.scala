...
sealed trait ベクトル集合[次元数 <: 自然数] {
  def +( vector: ベクトル集合[次元数] ): ベクトル集合[次元数]
  def toString: String
  def numStr: String
  def ++[任意の次元数 <: 自然数]( vector: ベクトル集合[任意の次元数] ): ベクトル集合[次元数#足す[任意の次元数]]
}
...
case object 零次元ベクトル extends ベクトル集合[零] {
  ...
  def numStr: String = ""
  def ++[Size <: 自然数]( vector: ベクトル集合[Size] ): ベクトル集合[零#足す[Size]] = vector
}
...
case class ベクトル[残り個数 <: 自然数]( 最初: Int, 残り: ベクトル集合[残り個数]) extends ベクトル集合[後者[残り個数]] {
  ...
  def ++[Size <: 自然数]( vector: ベクトル集合[Size] ): ベクトル集合[後者[残り個数]#足す[Size]] = 
    new ベクトル[残り個数#足す[Size]]( 最初, 残り.++( vector ) )
}
...
object ベクトル extends Dynamic {
  ...
  def apply[次元数 <: 自然数]( numbers: Int* ): ベクトル集合[次元数] = macro ベクトルマクロ実装[次元数]
